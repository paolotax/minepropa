...
@adozioni.each do |appunto|
  ...
  pdf.float do
    pdf.bounding_box [pdf.bounds.width / 2.0, pdf.bounds.top], :width => 300 do
      pdf.move_down(200)
      pdf.text appunto.classe.to_s, :size => 45, :spacing => 4, :style => :bold
      
      if !appunto.classe.scuola.indirizzi.empty?
        indirizzo = appunto.classe.scuola.indirizzi.first
        pdf.text indirizzo.destinatario,  :size => 14, :style => :bold, :spacing => 4
        pdf.text indirizzo.indirizzo
        pdf.text indirizzo.cap + ' ' + indirizzo.citta + ' ' + indirizzo.provincia
        ...
        pdf.text appunto.classe.scuola.nome_scuola, :size => 14, :style => :bold, :spacing => 16
        pdf.text appunto.classe.scuola.citta + " " + appunto.classe.scuola.provincia, :size => 12
      end
    end
  end
  # 
  pdf.move_down(400)
  pdf.text "Materiale abbinato al testo: #{appunto.libro.titolo}", :size => 14
  # pdf.move_down(20)
  ...
  pdf.start_new_page unless appunto == @adozioni.last
end